...
class BRIDGE_Model(nn.Module):
    ...
    def dynamic_padding(self, q, r, a):
        ...
        for bsz_i in range(bsz):
            ...
            sen_1_len = len(n_qra) - 1 - len(n_a_c)
            seg_mask[bsz_i, :(len(n_q_c) + 2)] *= 0
            seg_mask[bsz_i, sen_1_len:len(n_qra)] *= 0
            qAr_mask[bsz_i, 1:(len(n_q_c) + 1)] = 1(n_q_c) + 1 = 1
            qAr_mask[bsz_i, (len(n_q_c) + 2):(sen_1_len - 1)] = 1
            qAr_mask[bsz_i, len(n_qra):(len(n_qra) + r.size(1) + q.size(1) - len(n_r_c) - len(n_q_c))] = 1
            ans_mask[bsz_i, sen_1_len - 1:len(n_qra)] = 1
    ...
    def memorygate(self, query, kv_memory, mem_encoder, gate_projector):
        zvalue, attn = mem_encoder(query, kv_memory, kv_memory)
        gate = gate_projector(torch.cat((zvalue, query), dim=2))
    ...
    def forward(self, qrah):
        ...
        for i in range(ans.size(1)):
            ...
            bridge_trgs.append(trg[ans_mask == 1])
        output = self.decoder(a_memory)
        for bridge_trg in bridge_trgs:
            lm_loss = self._compute_loss(output, bridge_trg, soft=False)
